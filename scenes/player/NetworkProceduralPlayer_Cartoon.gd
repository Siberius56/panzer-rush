# Cartoon procedural player v3. Adds explicit mid-step knee bend during walk.
# Cartoon proportion variant generated from NetworkProceduralPlayer.gd
# Logic intentionally kept the same; the visual and proportion changes are in the companion .tscn.
...
@export var step_height: float = .22 # 0.16
@export var gait_frequency: float = 11.0
@export var foot_ground_offset: float = 0.035
@export var max_leg_reach: float = 1.0
@export_range(0.0, 0.3, 0.01) var leg_knee_bend_reserve: float = 0.0 # 0.08
@export_range(0.5, 1.0, 0.01) var grounded_leg_length_ratio: float = 0.86 # 0.74
@export var walk_knee_bend_forward: float = 0.32 # 0.24
@export var walk_knee_bend_up: float = 0.04 #0.035
@export_range(0.0, 1.0, 0.01) var walk_knee_bend_min_speed_ratio: float = 0.04 #  0.05
...
@export var foot_follow_speed: float = 14 # 18.0
...
var left_walk_knee_bend: float = 0.0
var right_walk_knee_bend: float = 0.0
...
func _sample_grounded_foot_position(side_sign: float, horizontal_velocity: Vector3, phase_offset: float, instant: bool = false) -> Vector3:
	...
	var knee_bend: float = float(foot_data.get("knee_bend", 0.0))

	if instant or speed_ratio < walk_knee_bend_min_speed_ratio:
	...
	if side_sign < 0.0:
		left_walk_knee_bend = knee_bend
	else:
		right_walk_knee_bend = knee_bend
...
func _get_foot_cycle_data(side_sign: float, move_direction: Vector3, speed_ratio: float, phase_value: float) -> Dictionary:
	...
	var knee_bend_factor: float = lift_phase * speed_ratio

	return {
		"local_x": local_x,
		"local_z": local_z,
		"lift": lift_amount,
		"knee_bend": knee_bend_factor
	}


func _update_air_feet(delta: float) -> void:
	left_walk_knee_bend = 0.0
	right_walk_knee_bend = 0.0
...
func _get_leg_total_length() -> float:
	return max(upper_leg_length + lower_leg_length, 0.001)


func _get_leg_effective_reach() -> float:
	var leg_total_length: float = _get_leg_total_length()
	var reserved_reach: float = max(0.0, min(leg_knee_bend_reserve, leg_total_length - 0.001))
	return max(0.001, min(max_leg_reach, leg_total_length - reserved_reach))
...
func _get_probe_ground_position(probe: RayCast3D, side_sign: float) -> Vector3:
	...
	var effective_leg_reach: float = _get_leg_effective_reach()
	var nominal_foot_y: float = hip_world_position.y - effective_leg_reach * grounded_leg_length_ratio
...
func _clamp_foot_target(side_sign: float, target_position: Vector3) -> Vector3:
	...
	var lowest_allowed_y: float = hip_world_position.y - effective_leg_reach
	...
	if target_distance > effective_leg_reach:
		var clamped_direction: Vector3 = hip_to_target.normalized()
		target_position = hip_world_position + clamped_direction * effective_leg_reach
		target_position.y = clamp(target_position.y, lowest_allowed_y, highest_allowed_y)
...
func _update_limbs() -> void:
	...
	var left_leg_bend_direction: Vector3 = (body_forward + body_right * -0.25).normalized()
	var right_leg_bend_direction: Vector3 = (body_forward + body_right * 0.25).normalized()

	var left_knee: Vector3 = _solve_two_bone_joint(
		left_hip.global_position,
		left_foot.global_position,
		left_leg_bend_direction,
		upper_leg_length,
		lower_leg_length
	)
	var right_knee: Vector3 = _solve_two_bone_joint(
		...
		right_leg_bend_direction,
		upper_leg_length,
		lower_leg_length
	)

	left_knee = _apply_walk_knee_bend(left_knee, left_leg_bend_direction, left_walk_knee_bend)
	right_knee = _apply_walk_knee_bend(right_knee, right_leg_bend_direction, right_walk_knee_bend)
...
func _apply_walk_knee_bend(knee_position: Vector3, bend_direction: Vector3, bend_factor: float) -> Vector3:
	var clamped_factor: float = clamp(bend_factor, 0.0, 1.0)
	if clamped_factor <= 0.001:
		return knee_position

	var safe_bend_direction: Vector3 = bend_direction
	if safe_bend_direction.length_squared() <= 0.0001:
		safe_bend_direction = Vector3.FORWARD
	safe_bend_direction = safe_bend_direction.normalized()

	var forward_offset: Vector3 = safe_bend_direction * walk_knee_bend_forward * clamped_factor
	var up_offset: Vector3 = Vector3.UP * walk_knee_bend_up * clamped_factor
	return knee_position + forward_offset + up_offset
