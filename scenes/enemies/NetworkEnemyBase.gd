...
@export var use_direct_move_when_close: bool = false
@export var direct_move_distance: float = 7.0
...
@export_range(1, 120, 1) var repath_interval_frames: int = 30
@export_range(1, 120, 1) var attack_check_interval_frames: int = 10
@export var randomize_scan_frame: bool = true
@export var throttle_path_target_updates: bool = true

@export_group("Target Manager")
@export var use_enemy_target_manager: bool = true
@export var target_manager_path: NodePath = NodePath("/root/EnemyTargetManager")

var _cached_target_manager: Node = null
...
@export var state_send_interval: float = 0.25
...
var _repath_frame_offset: int = 0
var _attack_check_frame_offset: int = 0
...
func _ready() -> void:
	...
	health = max_health
	attack_timer = randf_range(0.0, maxf(attack_cooldown, 0.01))
...
func _configure_scan_schedule() -> void:
	var scan_interval: int = max(scan_interval_frames, 1)
	var repath_interval: int = max(repath_interval_frames, 1)
	var attack_interval: int = max(attack_check_interval_frames, 1)

	if randomize_scan_frame:
		_scan_frame_offset = randi() % scan_interval
		_repath_frame_offset = randi() % repath_interval
		_attack_check_frame_offset = randi() % attack_interval
	else:
		var instance_offset: int = int(get_instance_id())
		_scan_frame_offset = instance_offset % scan_interval
		_repath_frame_offset = instance_offset % repath_interval
		_attack_check_frame_offset = instance_offset % attack_interval
...
func _should_run_scheduled_repath() -> bool:
	var interval: int = max(repath_interval_frames, 1)
	var frame: int = int(Engine.get_physics_frames())
	return frame % interval == _repath_frame_offset % interval


func _should_run_scheduled_attack_check() -> bool:
	var interval: int = max(attack_check_interval_frames, 1)
	var frame: int = int(Engine.get_physics_frames())
	return frame % interval == _attack_check_frame_offset % interval
...
func _can_refresh_path_target() -> bool:
	...
	if not _has_nav_target:
		return true
	return _should_run_scheduled_repath()
...
func _get_attack_range() -> float:
	if is_melee_attack:
		return minf(attack_range, 1.8)
	return attack_range


func _should_use_attack_line_of_sight() -> bool:
	return attack_requires_line_of_sight


func _should_use_detection_line_of_sight() -> bool:
	return detection_requires_line_of_sight
...
func _handle_combat_state(combat_target: Node3D, should_run_scheduled_scan: bool) -> void:
	...
	var distance_squared: float = global_position.distance_squared_to(combat_target.global_position)
	var desired_attack_range: float = _get_attack_range()
	var desired_attack_range_squared: float = desired_attack_range * desired_attack_range
	var has_los: bool = true
	if _should_use_attack_line_of_sight():
	...
	if distance_squared <= desired_attack_range_squared and has_los:
		_set_state(EnemyState.ATTACK)
		velocity.x = 0.0
		velocity.z = 0.0
		_clear_navigation_target()
		if _should_run_scheduled_attack_check():
		...
		if not has_los:
			...
			if distance_squared <= desired_attack_range_squared:
				_attack_debug("attack blocked by line of sight, keep moving toward target=%s range=%.2f" % [combat_target.name, desired_attack_range])
...
func _move_towards(target_position: Vector3, stop_distance: float = 0.0) -> void:
	...
	var planar_distance_squared: float = planar_to_target.length_squared()
	var safe_stop_distance: float = maxf(stop_distance, 0.05)
	if planar_distance_squared <= safe_stop_distance * safe_stop_distance:
		velocity.x = 0.0
		velocity.z = 0.0
		return
	...
	var direct_move_distance_squared: float = maxf(direct_move_distance, 0.0) * maxf(direct_move_distance, 0.0)
	if use_direct_move_when_close and planar_distance_squared <= direct_move_distance_squared:
	...
	var repath_distance_squared: float = repath_distance * repath_distance
	var needs_new_path: bool = not _has_nav_target or _last_nav_target.distance_squared_to(flat_target) > repath_distance_squared
...
func _try_attack(combat_target: Node3D, target_point: Vector3) -> void:
	...
	var attack_succeeded: bool = _perform_attack(combat_target, target_point)
...
func _perform_attack(_combat_target: Node3D, _target_point: Vector3) -> bool:
	return false
...
func _find_nearest_target(max_distance: float = INF) -> Node3D:
	...
	if use_enemy_target_manager and target_manager != null and target_manager.has_method("get_nearest_target"):
		# Hot path. Do not pass `self` to the manager.
		# The previous version called back into _is_valid_target() for every enemy query,
		# which triggered expensive property-list scans on every target.
		var manager_target: Variant = target_manager.call("get_nearest_target", global_position, max_distance)
...
func _get_target_manager() -> Node:
	if not use_enemy_target_manager:
		return null
	if _cached_target_manager != null and is_instance_valid(_cached_target_manager):
		return _cached_target_manager
	if get_tree() == null:
		return null

	var manager: Node = get_node_or_null(target_manager_path)
	if manager != null and is_instance_valid(manager):
		_cached_target_manager = manager
		return manager

	manager = get_tree().root.get_node_or_null("EnemyTargetManager")
	if manager != null and is_instance_valid(manager):
		_cached_target_manager = manager
		return manager

	var managers: Array = get_tree().get_nodes_in_group("enemy_target_manager")
	if not managers.is_empty():
		var found_manager: Node = managers[0] as Node
		if found_manager != null and is_instance_valid(found_manager):
			_cached_target_manager = found_manager
			return found_manager

	if multiplayer.is_server():
		var created_manager: Node = EnemyTargetManager.new()
		created_manager.name = "EnemyTargetManager"
		get_tree().root.add_child(created_manager)
		_cached_target_manager = created_manager
...
func _validate_current_target() -> void:
	...
	if global_position.distance_squared_to(current_target.global_position) > max_target_distance_squared:
...
func _can_detect_target(target: Node3D) -> bool:
	if not _is_valid_target(target):
		return false
	if _should_use_detection_line_of_sight() and not _has_detection_line_of_sight(target):
		return false
	return true
...
func _is_target_dead(node: Node) -> bool:
	...
	# Fast path for this project. Player and Vehicle both expose `is_dead`.
	# This avoids _safe_get_property() and therefore avoids get_property_list() scans.
	var dead_value: Variant = node.get("is_dead")
	if dead_value is bool:
		return bool(dead_value)
...
func _safe_get_property(object: Object, property_name: String) -> Variant:
	...
	# Important: this function is used in hot paths.
	# Never call get_property_list() here. It allocates and is extremely expensive
	# when 80+ enemies validate targets every physics frame.
...
func _try_retarget_from_damage_source(source: Node) -> void:
	...
	var damage_retarget_max_distance_squared: float = damage_retarget_max_distance * damage_retarget_max_distance
	if global_position.distance_squared_to(attacker.global_position) > damage_retarget_max_distance_squared:
		return
...
func _on_ally_death_reported(dead_ally: Node, attacker: Node3D) -> void:
	...
	if max_alert_distance > 0.0 and global_position.distance_squared_to(dead_ally.global_position) > max_alert_distance * max_alert_distance:
		return
