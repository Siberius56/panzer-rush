...
func set_open(value: bool, immediate: bool = false) -> void:
	...
	var to_animate := animation_duration
	
	if immediate or animation_duration <= 0.0:
		to_animate = .01
	
	var tween: Tween = create_tween().set_parallel(true).set_ease(tween_ease).set_trans(tween_transition)
	_active_tween = tween

	if left_panel != null:
		tween.tween_property(left_panel, "position:x", -to_move_door_x, to_animate)

	if right_panel != null:
		tween.tween_property(right_panel, "position:x", to_move_door_x, to_animate)
